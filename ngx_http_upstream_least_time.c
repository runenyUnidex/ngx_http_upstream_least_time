...
ngx_http_upstream_get_least_time_peer(ngx_peer_connection_t *pc, void *data)
{
         ...
         peer = peer->next, i++)
    {
        ...
        if (peer->rr->max_fails && peer->rr->fails >= peer->rr->max_fails && now - peer->rr->checked <= peer->rr->fail_timeout)
        {
            continue;
        }
         ...
         */
        if (best == NULL || peer->rr->conns * best->rr->weight < best->rr->conns * peer->rr->weight)
        {
            best = peer;
            many = 0;
            p = i;
...
void ngx_http_upstream_free_least_time_peer(ngx_peer_connection_t *pc, void *data,
                                            ngx_uint_t state)
{
    ngx_http_upstream_least_time_peer_data_t *ltp = data;
...
ngx_http_upstream_least_time(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ltcf->config != (ngx_uint_t)NGX_CONF_UNSET)
    {
        return "is duplicate";
    }
    ...
    if ((cf->args->nelts < 2) || (cf->args->nelts > 3) || (ltcf->config == (ngx_uint_t)NGX_CONF_UNSET))
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid format in \"%V\" directive", &cmd->name);
        return NGX_CONF_ERROR;
    }
